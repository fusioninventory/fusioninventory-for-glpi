...
redipsInit = function (order_type,order_subtype,order_id) {
   ...
   rd.event.rowDroppedBefore = function (sourceTable, sourceRowIndex) {
      ...
      var orders_id_search = form.children("input[name='orders_id']");

      if (itemtype_search.length == 1 ) itemtype = itemtype_search.val();
      if (orders_id_search.length == 1 ) orders_id = orders_id_search.val();

      if ( (typeof orders_id === 'undefined') || (typeof itemtype === 'undefined') ) {
         return false;
      }
...
//            var data = Ext.decode( result.responseText );
            ...
            var els = jQuery('#package_order_'+ orders_id +' #package_json_debug');
