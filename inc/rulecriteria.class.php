<?php
/*
 * @version $Id: rulecriteria.class.php 13552 2011-01-10 09:13:36Z yllen $
 -------------------------------------------------------------------------
...
/// Criteria Rule class
class PluginFusioninventoryRuleCriteria extends CommonDBChild {
   ...
   function __construct($rule_type='Rule') {
      ...
      $this->forceTable('glpi_rulecriterias');
   }
   ...
   **/
   function getRuleCriterias($ID) {
      ...
      while ($rule = $DB->fetch_assoc($result)) {
         $tmp          = new PluginFusioninventoryRuleCriteria;
         $tmp->fields  = $rule;
         $rules_list[] = $tmp;
      }
   ...
   **/
   function getValueToMatch($condition, &$initValue) {
      ...
      if (!empty($type)
          && ($condition!=PluginFusioninventoryRule::PATTERN_IS && $condition!=PluginFusioninventoryRule::PATTERN_IS_NOT)) {
   ...
   **/
   static function match(pluginfusioninventoryRuleCriteria &$criterion, $field, &$criterias_results, &$regex_result) {
      ...
      if ($pattern == PluginFusioninventoryRule::RULE_WILDCARD || $pattern == PluginFusioninventoryRule::PATTERN_FIND) {
         return true;
      }
      ...
      if ($condition != PluginFusioninventoryRule::REGEX_MATCH && $condition != PluginFusioninventoryRule::REGEX_NOT_MATCH) {
      ...
      switch ($condition) {
         ...
         case PluginFusioninventoryRule::PATTERN_IS :
            if (is_array($field)) {
         ...
         case PluginFusioninventoryRule::PATTERN_IS_NOT :
            if ($field != $pattern) {
               $criterias_results[$criteria] = $pattern;
               return true;
            }
         ...
         case PluginFusioninventoryRule::PATTERN_END :
            $value = "/".$pattern."$/";
         ...
         case PluginFusioninventoryRule::PATTERN_BEGIN :
            if (empty($pattern)) {
               return false;
            }
         ...
         case PluginFusioninventoryRule::PATTERN_CONTAIN :
            if (empty($pattern)) {
               return false;
            }
            $value = strpos($field,$pattern);
         ...
         case PluginFusioninventoryRule::PATTERN_NOT_CONTAIN :
            if (empty($pattern)) {
               return false;
            }
            $value = strpos($field,$pattern);
         ...
         case PluginFusioninventoryRule::REGEX_MATCH :
            $results = array();
         ...
         case PluginFusioninventoryRule::REGEX_NOT_MATCH :
   ...
   **/
   static function getConditions($itemtype, $criterion = '') {
      ...
      $criteria =  array(PluginFusioninventoryRule::PATTERN_IS              => $LANG['rulesengine'][0],
                         PluginFusioninventoryRule::PATTERN_IS_NOT          => $LANG['rulesengine'][1],
                         PluginFusioninventoryRule::PATTERN_CONTAIN         => $LANG['rulesengine'][2],
                         PluginFusioninventoryRule::PATTERN_NOT_CONTAIN     => $LANG['rulesengine'][3],
                         PluginFusioninventoryRule::PATTERN_BEGIN           => $LANG['rulesengine'][4],
                         PluginFusioninventoryRule::PATTERN_END             => $LANG['rulesengine'][5],
                         PluginFusioninventoryRule::REGEX_MATCH             => $LANG['rulesengine'][26],
                         PluginFusioninventoryRule::REGEX_NOT_MATCH         => $LANG['rulesengine'][27],
                         PluginFusioninventoryRule::PATTERN_EXISTS          => $LANG['plugin_fusioninventory']['rules'][9],
                         PluginFusioninventoryRule::PATTERN_DOES_NOT_EXISTS => $LANG['plugin_fusioninventory']['rules'][10]);
   ...
   **/
   static function dropdownConditions($itemtype, $params=array()) {
      ...
      $elements = array();
      foreach (PluginFusioninventoryRuleCriteria::getConditions($itemtype, $p['criterion']) as $pattern => $label) {
