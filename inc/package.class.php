   ...
   function downloadFragments ($fileSha1sum) {
      ...
      if (isset($numFragment)) {
         ...
         if ($numFragment == $this->fields['fragments']) {
            echo file_get_contents(GLPI_PLUGIN_DOC_DIR."/fusinvdeploy/packages/".$fileSha1sum, NULL, NULL, $fragmentSize * ($numFragment - 1));
         } else {
            echo file_get_contents(GLPI_PLUGIN_DOC_DIR."/fusinvdeploy/packages/".$fileSha1sum, NULL, NULL, $fragmentSize * ($numFragment - 1), $fragmentSize);
         }
      } else {
         // send all file
