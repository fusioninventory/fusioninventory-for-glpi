...
function plugin_fusioninventory_is_hex($value) {
	$hex=0;
	$len = strlen($value);
...
function plugin_fusioninventory_stringToIfmac($string) {
	if ($string == "") {
		return false;
   }
	...
	$i=0;
	$null=0;
	while (($i<6)  && ( ($is_hex = plugin_fusioninventory_is_hex($string["$i"])) != false )) {
		if ($string["$i"] == "00") {
			$null++;
      }
		$i++;
...
function plugin_fusioninventory_ifmacToDecimal($ifmac) {
