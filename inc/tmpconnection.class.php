...
class PluginTrackerTmpConnections extends CommonDBTM {
	...
	function WireInterSwitchs($PID) {
      ...
      $query = "SELECT *
                FROM `glpi_plugin_tracker_tmp_connections`
      ...
      while ($data=$DB->fetch_array($result)) {
         $query_sel1 = "SELECT *
                        FROM `glpi_plugin_tracker_tmp_netports`
                             LEFT JOIN `glpi_networking_ports`
                                       ON `glpi_networking_ports`.`ID` = `FK_networking_port`
		...
		$query = "SELECT `ifmac`, `glpi_plugin_tracker_tmp_netports`.`ID`
                FROM `glpi_plugin_tracker_tmp_netports`
                     LEFT JOIN `glpi_networking_ports`
                               ON `glpi_networking_ports`.`ID`=`FK_networking_port`
                WHERE `cdp`='1';";
		...
		while ($i != 0) {
			$i = 0;
         $query = "SELECT `macaddress`, `glpi_plugin_tracker_tmp_netports`.`ID`, `FK_networking`,
                          `FK_networking_port`
                   FROM `glpi_plugin_tracker_tmp_netports`
                        LEFT JOIN `glpi_plugin_tracker_tmp_connections`
                                  ON `glpi_plugin_tracker_tmp_connections`.`FK_tmp_netports `=
                                     `glpi_plugin_tracker_tmp_netports`.`ID`
                   WHERE `macaddress` IS NOT NULL
...
//            }
               ...
               if ($_SESSION['tracker_logs'] == "1")
                  $logs->write("tracker_fullsync","Connection wire switch ".$sport." - ".$dport,
                               "","",1);
                  ...
                  if ($DB->numrows($result_sel2) != "0") {
                     ...
                     $DB->query($query_delete);
                     // Delete in glpi_plugin_tracker_tmp_connections to tmp_netports ports
                     // destination
                     $query_delete = "DELETE FROM `glpi_plugin_tracker_tmp_connections`
