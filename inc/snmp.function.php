 ...
 *
**/
function plugin_fusioninventory_UpdateDeviceBySNMP_startprocess(
 ...
 *
**/
function plugin_fusioninventory_UpdateDeviceBySNMP_process(
   ...
   unset($device);
	...
	if ((isset($oidsModel)) && ($ID_Device != "")) {
      ...
      if (strstr($oidvalues[".1.3.6.1.2.1.1.1.0"][""],"Cisco")) {
		...
		$ArrayPort_Object_oid = plugin_fusioninventory_snmp_GetOIDPorts(
		...
		if ($type == NETWORKING_TYPE) {
			plugin_fusioninventory_snmp_networking_ifaddr($ID_Device,$type,$oidsModel,$oidvalues);
      }

		// ** Update fields of switchs
		plugin_fusioninventory_snmp_UpdateGLPIDevice(
		...
		if (!empty($ArrayPort_Object_oid)) {
			plugin_fusioninventory_UpdateGLPINetworkingPorts(
            $ID_Device,$type,$oidsModel,$oidvalues,$Array_Object_TypeNameConstant);
      }
      $Array_trunk_ifIndex = array();

		if ($type == NETWORKING_TYPE) {
			$Array_trunk_ifIndex = plugin_fusioninventory_cdp_trunk(
		...
		if (!empty($ArrayPort_Object_oid)) {
			$array_port_trunk = plugin_fusioninventory_GetMACtoPort(
		...
		if (($type == NETWORKING_TYPE) AND ($sysDescr == "Cisco")) {
			...
			foreach ($Array_vlan as $num=>$vlan_ID) {
				plugin_fusioninventory_GetMACtoPort(
               $ID_Device,$type,$oidsModel,$oidvalues,$array_port_trunk,$ArrayPortDB_Name_ID,
 ...
 *
**/
function plugin_fusioninventory_snmp_GetOIDPorts(
      $ID_Device,$type,$oidsModel,$oidvalues,$ArrayPort_LogicalNum_SNMPName,
      $ArrayPort_LogicalNum_SNMPNum) {
   global $DB,$LANG;
 ...
 *
**/
function plugin_fusioninventory_snmp_UpdateGLPIDevice(
      $ID_Device,$type,$oidsModel,$oidvalues,$Array_Object_TypeNameConstant) {
		...
		if (!preg_match("/\.$/",$oid)) { // SNMPGet ONLY
         if (isset($oidvalues[$oid][""])) {
            if ((isset($FUSIONINVENTORY_MAPPING[$type][$link]['dropdown']))
               ...
               $oidvalues[$oid][""] = plugin_fusioninventory_hex_to_string($oidvalues[$oid][""]);
 ...
 *
**/
function plugin_fusioninventory_UpdateGLPINetworkingPorts(
      $ID_Device,$type,$oidsModel,$oidvalues,$Array_Object_TypeNameConstant) {
...
//			print "OID : ".$oid."\n";
			...
			while ($data=$DB->fetch_array($result)) {
               ...
               if ($queryUpdate != '') {
                  ...
                  plugin_fusioninventory_snmp_addLog($data["ID"],$FUSIONINVENTORY_MAPPING[$type][$link]['name'],
 ...
 *
**/
function plugin_fusioninventory_GetMACtoPort($ID_Device,$type,$oidsModel,$oidvalues,$array_port_trunk,
 ...
 *
**/
function plugin_fusioninventory_cdp_trunk($ID_Device,$type,$oidsModel,$oidvalues,
      ...
      while ($data=$DB->fetch_array($result)) {
         ...
         if ((isset($Array_trunk_ifIndex[$ifIndex])) AND ($Array_trunk_ifIndex[$ifIndex] == "1")) {
            if ($data['trunk'] != "1") {
               $query_update = "UPDATE `glpi_plugin_fusioninventory_networking_ports`
                                ...
                                WHERE `ID`='".$data['sid']."';";
               $DB->query($query_update);
               plugin_fusioninventory_snmp_addLog(
         ...
         // If multiple => -1
            ...
            $query_update = "UPDATE `glpi_plugin_fusioninventory_networking_ports`
                             ...
                             WHERE `ID`='".$data['sid']."';";
            $DB->query($query_update);
            plugin_fusioninventory_snmp_addLog($data["FK_networking_ports"],"trunk","1","0","",
               $_SESSION['FK_process']);
         }
      }
   }
...
function plugin_fusioninventory_snmp_networking_ifaddr($ID_Device,$type,$oidsModel,$oidvalues) {
	global $DB;
