...
//                        $printer_select = $data;
  ...
  function testNetwork($xml='', $items_id=0, $unknown=0, $xmlFile='') {
      ...
      foreach ($xml->CONTENT->NETWORKS as $child) {
         if (isset($child->DESCRIPTION)) {
            ...
            if (isset($child->MACADDR)) {
               if (isset($networkunique[(string)$child->DESCRIPTION."-".(string)$child->MACADDR])) {
                  $add = 0;
               }
            } else {
               if (isset($networkunique[(string)$child->DESCRIPTION])) {
                  $add = 0;
               }
            }
            if ($add == 1) {
               ...
               $i++;
               if (isset($child->MACADDR)) {
                  $networkunique[(string)$child->DESCRIPTION."-".(string)$child->MACADDR] = 1;
               } else {
                  $networkunique[(string)$child->DESCRIPTION] = 1;
               }
            }
   ...
   function testSoftware($xmlFile='', $items_id=0, $unknown=0) {
      ...
      $this->assertEquals($DB->numrows($result), (count($a_softwareXML)), 'Difference of Softwares, created '.$DB->numrows($result).' times instead '.(count($a_softwareXML)).' ['.$xmlFile.']'.print_r($a_diff, true).' or '.print_r($a_diff2, true));
