...
if (isset ($_POST["add"])) {
   ...
   $query_ins = "INSERT INTO `glpi_plugin_tracker_construct_walks` (
`ID` ,
`construct_device_id` ,
`log`
)
...
NULL , '".$_POST['ID']."', '".$md5."'
   ...
   move_uploaded_file($_FILES['walk']['tmp_name'], GLPI_PLUGIN_DOC_DIR."/tracker/walks/".$md5);
   glpi_header($_SERVER['HTTP_REFERER']);
} else if (isset($_POST['mib'])) { // Check MIBS
   foreach($_POST['oidsselected'] as $oid) {
      $a_mapping = explode('||', $_POST['links_oid_fields_'.$oid]);

      $query_ins = "INSERT INTO glpi_plugin_tracker_construct_mibs
   ...
   glpi_header($_SERVER['HTTP_REFERER']);
	...
	$ptcd->delete($_POST);
	glpi_header("plugin_tracker.construct_device.php");
}
