...
class PluginFusionInventoryManufacturerCisco extends CommonDBTM {
               ...
               switch (!false) {
                  ...
                  case strstr($oidvalues[".1.3.6.1.2.1.1.1.0"][""],"Cisco") :
                     if ((isset($oidvalues[$oidsModel[0][1]['vlanTrunkPortDynamicStatus'].".".$snmpportID][$vlan]))
                        ...
                        if ($_SESSION['fusioninventory_logs'] == "1") $logs->write("fusioninventory_fullsync","Trunk = ".$snmpportID,$type,$ID_Device,1);
         ...
         foreach($Array_trunk_IP_hex_result AS $num=>$snmpportID) {
            ...
            if (substr_count($ip_switch_trunk,'.') == 3) {
              ...
              // $Array_trunk_ifIndex[$ifIndex] = 1;
               ...
               if ($_SESSION['fusioninventory_logs'] == "1") $logs->write("fusioninventory_fullsync","ifIndex = ".$ifIndex,$type,$ID_Device,1);

               // Search port of switch connected on this port and connect it if not connected
               if ($_SESSION['fusioninventory_logs'] == "1") $logs->write("fusioninventory_fullsync","ip = ".$ip_switch_trunk." / ifdescr = ".$oidvalues[$oidsModel[0][1]['cdpCacheDevicePort'].".".$snmpportID][""],$type,$ID_Device,1);
   ...
   function GetMACtoPort($ID_Device,$type,$oidsModel,$oidvalues,$array_port_trunk,$ArrayPortsID,$vlan,$Array_trunk_ifIndex) {
      ...
      foreach($ArrayMACAdressTable as $num=>$dynamicdata) {
         ...
         if (((count($oidExplode) > 3)) AND (isset($oidvalues[$oidsModel[0][1]['dot1dTpFdbPort'].".".$dynamicdata][$vlan]))) {
            ...
            if ($_SESSION['fusioninventory_logs'] == "1") $logs->write("fusioninventory_fullsync","****************",$type,$ID_Device,1);
            if ($_SESSION['fusioninventory_logs'] == "1") $logs->write("fusioninventory_fullsync","BRIDGEPortNumber = ".$BridgePortNumber,$type,$ID_Device,1);
