...
class ImportRuleTest extends RestoreDatabase_TestCase {
   ...
   function activeRule($name) {
      ...
      $rules = $pfRule->find("`name` LIKE '%".$name."%' AND `is_active`=0");
   ...
   function addRule($name, $criteria=[], $action=[], $afterRuleName='') {
      ...
      $DB->query("UPDATE glpi_rules SET `ranking`=".$input['ranking']
            . " WHERE `id`=".$rules_id);
   ...
   public function updateIPIfdescrRestrictport() {
      ...
      $this->assertEquals(1, $this->items_id);
      $this->assertEquals('NetworkEquipment', $this->itemtype);
      $this->assertEquals(1, $this->ports_id);
    ...
    */
   public function searchMac_nomoredata() {
   ...
   public function rulepassedNetworkEquipment_nodevice() {
      ...
      $port = current($ports);
      $this->assertEquals("PluginFusioninventoryUnmanaged", $port['itemtype']);
   }
