...
function plugin_headings_fusioninventory_tasks($item, $itemtype='', $items_id=0) {
   ...
   if ($itemtype == 'Computer') {
      ...
      $allowed = PluginFusioninventoryTaskjob::isAllowurlfopen(1);
      if ($allowed) {
         $pfAgent = new PluginFusioninventoryAgent();
...
function plugin_headings_fusioninventory_xml($item) {
   ...
   if ($xml != '') {
      ...
      echo " (".__('Last inventory', 'fusioninventory')."&nbsp;: ".
               Html::convDateTime(date("Y-m-d H:i:s", filemtime($filename))).")";
      echo "</th>";
      echo "</tr>";
...
function plugin_headings_fusioninventory_printerHistory($type, $id) {
   ...
   $print_history->showForm($_GET["id"],
               array('target'=>$CFG_GLPI['root_doc'].
                                  '/plugins/fusinvsnmp/front/printer_history.form.php'));
}
